...
> [!NOTE]
> **功能 2 的 CVE 資料庫來源** — ✅ 已決定
> 設備為 air-gapped 設計，不主動對接外部漏洞資料庫（NIST NVD、ClamAV 等）。即便未來接入 LIS 也不會連外。安全掃描以本地副檔名黑白名單 + 精確檔名白名單為基礎機制。

> [!NOTE]
> **功能 1 的檔案系統格式** — ✅ 已決定使用 `exFAT`
> - 無 4GB 單檔限制（FAT32 天花板）
> - Windows Vista SP1+ 原生支援，跨平台相容
> - 快速格式化指令：`format E: /FS:exFAT /Q /Y`
