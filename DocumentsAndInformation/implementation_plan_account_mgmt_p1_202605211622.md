...
## Open Questions（Part 1 修訂後剩餘）

> [!IMPORTANT]
> **Q-P1-3：ServiceModePage 子頁面導覽方式？**
>
> Machine Setting / Flow Setting 未來可能有子功能層次（如 Machine Setting → Flow List / Other）。
> 建議兩種方案：
> - **選項 A**：在 AppShell 中繼續以 `NavigateTo()` 處理子頁面路由（扁平化）
> - **選項 B**：ServiceModePage 內部自行管理子頁面切換（內嵌 Frame）
>
> **建議**：選項 B（ServiceModePage 內嵌切換），避免 AppShell 路由表過於龐大。
> 本次先建殼，待功能開發時再決定。**可暫不決議。**
