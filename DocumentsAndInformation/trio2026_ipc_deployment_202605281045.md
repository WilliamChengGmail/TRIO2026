...
> [!NOTE]
> 軟體使用 **Viewbox 等比縮放**，UI 會自動適配不同解析度面板，無需修改程式碼。
> 更大面板 = 更大的按鍵觸控區域 + 更清晰的文字，操作體驗直接提升。

#### 面板尺寸選型對照

| 尺寸 | 可視面積 (mm) | 典型解析度 | 旋轉直式後 | 適用場景 | 推薦度 |
|------|-------------|-----------|-----------|---------|--------|
| **7"** | 154 × 86 | 1024 × 600 | 600 × 1024 | 嵌入式儀器面板、空間受限 | ★★★ |
| **10.1"** | 218 × 136 | 1280 × 800 | 800 × 1280 | **通用首選**、實驗室桌面 | ★★★★★ |
| **12.1"** | 261 × 163 | 1024 × 768 / 1280 × 800 | 768 × 1024 / 800 × 1280 | 獨立工作站、潔淨室壁掛 | ★★★★ |
| **15.6"** | 345 × 194 | 1920 × 1080 | 1080 × 1920 | 中控監控台、多工操作 | ★★★★ |

> [!IMPORTANT]
> **機構升級建議**：若機構空間允許，**10.1 吋為最佳升級選擇**：
> - 觸控面積比 7 吋增加 **約 2.5 倍**，手套操作大幅改善
> - 1280×800 解析度旋轉後 800×1280，與設計基準 600×960 比例接近，幾乎無黑邊
> - 工業面板電腦選項最豐富、價格合理
> - 按鍵有效點擊面積從 ~5mm 提升至 ~7mm，符合 IEC 62366 醫療 HFE 觸控標準

#### 面板硬體參數需求

| 參數 | 最低要求 (7") | 推薦規格 (10.1"+) | 說明 |
|------|-------------|-------------------|------|
| **尺寸** | 7 吋 | **10.1 吋** ~ 15.6 吋 | 依機構空間決定 |
| **方向** | Portrait (直式) | Portrait (直式) | 可透過 OS 旋轉橫式面板 |
| **解析度** | 800 × 480 (WVGA) | **1280 × 800** (WXGA+) | Viewbox 自動縮放 |
| **像素密度** | ~133 PPI | ≥ 150 PPI | 大面板不需過高 PPI |
| **觸控類型** | 電阻式 (Resistive) | **PCAP 電容式** | 多點觸控 + 手套 |
| **觸控點數** | 單點 | ≥ 5 點 | 預留手勢擴充 |
| **手套支援** | — | ✅ 必要 | 醫療/實驗室操作 |
| **亮度** | 300 cd/m² | ≥ 400 cd/m² | 明亮環境使用 |
| **對比度** | 500:1 | ≥ 800:1 | 文字/圖示辨識度 |
| **視角** | TN (±60°) | **IPS (±85°)** | 大面板更需要廣視角 |
| **背光壽命** | 30,000 hr | ≥ 50,000 hr | 24/7 運行約 5.7 年 |
| **表面處理** | 亮面 | **霧面 (AG)** | 減少反光 + 抗指紋 |
| **光學貼合** | Air Gap | **Optical Bonding** | 減少視差，大面板效果更明顯 |
| **介面** | LVDS | LVDS / eDP / HDMI | 大面板多用 eDP 或 HDMI |
| **前面板防護** | — | **IP65** | 防塵防水濺 |

#### 解析度適配對照表

由於 Viewbox 等比縮放 (設計基準 600×960，比例 5:8)，不同面板呈現效果如下：

| 面板尺寸 | 原始解析度 | 旋轉直式後 | 比例 | 黑邊位置 | UI 呈現 |
|---------|-----------|-----------|------|---------|---------|
| 7" | 800 × 480 | 480 × 800 | 3:5 | 上下黑邊 | 可用但偏小 |
| **7"** | **1024 × 600** | **600 × 1024** | **~5:8.5** | 上下極小 | **7 吋最佳** |
| **10.1"** | **1280 × 800** | **800 × 1280** | **5:8** | **幾乎無** | **★ 最佳匹配** |
| 10.1" | 1920 × 1200 | 1200 × 1920 | 5:8 | 無黑邊 | 極清晰 |
| 12.1" | 1024 × 768 | 768 × 1024 | 3:4 | 左右黑邊 | 可用 |
| 12.1" | 1280 × 800 | 800 × 1280 | 5:8 | 幾乎無 | 優秀 |
| 15.6" | 1920 × 1080 | 1080 × 1920 | 9:16 | 左右黑邊 | 清晰但有邊距 |
| 15.6" | 1200 × 1920 (直式) | 原生直式 | 5:8 | 無黑邊 | 完美 |

> [!TIP]
> **比例匹配關鍵**：設計基準為 5:8 (600:960)。選擇 **5:8 比例的面板**（如 1280×800、1200×1920）可獲得零黑邊全螢幕體驗。
> 16:9 面板 (1920×1080) 旋轉後為 9:16，與 5:8 略有差異，會在左右產生小邊距。

#### 各尺寸觸控體驗比較

| 項目 | 7 吋 | 10.1 吋 | 12.1 吋 | 15.6 吋 |
|------|------|---------|---------|---------|
| 按鍵有效面積 (52px 基準) | ~5 mm | ~7 mm | ~8.5 mm | ~11 mm |
| 觸控鍵盤單鍵 | 偏小，需精確 | **舒適** | 寬裕 | 接近實體鍵盤 |
| 手套操作性 | 勉強 | **良好** | 優秀 | 優秀 |
| 數字鍵盤 | 可用 | 舒適 | 寬裕 | 過大 |
| 閱讀舒適度 (20px 字體) | ~2 mm | ~2.8 mm | ~3.4 mm | ~4.5 mm |
| 站立操作距離 | 30 cm | 40 cm | 50 cm | 60 cm |
| 機構開孔 (約) | 190 × 120 mm | 260 × 170 mm | 300 × 200 mm | 390 × 240 mm |
...
| 品牌 | 7" 型號 | 10.1" 型號 | 12.1"+ 型號 | 特點 |
|------|---------|-----------|-------------|------|
| **Advantech (研華)** | TPC-71W | TPC-110W / PPC-3100 | PPC-3120 / PPC-3150 | 台灣品牌，完整產品線 |
| **Cincoze (德承)** | — | P1201 | P2102 | 無風扇，寬溫寬壓 |
| **Axiomtek (艾訊)** | GOT107W | GOT110 | GOT-3126T | 醫療認證版本可選 |
| **Kontron** | — | KBox A-150-WKL | FlatClient | 歐洲品牌，高可靠度 |
| **AAEON (研揚)** | AFOLUX | AHP-1100 | AHP-1121 / 1153 | 價格合理，產品線廣 |
