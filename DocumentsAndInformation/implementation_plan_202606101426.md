...
| 問題 | 回覆 | 設計影響 |
|------|------|---------|
| **IoT 版本** | 尚未採購，預計半年內與工業電腦一併採購 | Phase 3 部署腳本暫作通用設計，支援 LTSC 2021/2024，待確認後微調 |
| **UV Firmware** | 需要特權，由其他同仁撰寫 HAL，未來整併 | PrivilegedService 預留 `UvControl` 命令類型，IPC 協議設計保持擴充性 |
| **遠端維護** | Air-gapped 設計，不開啟遠端服務；未來可能串接 LIS | 不設定 RDP/SSH，部署腳本中移除相關設定；Service IPC 僅限 localhost Named Pipe |
